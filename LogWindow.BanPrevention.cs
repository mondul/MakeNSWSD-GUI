using System;
using System.Collections.Generic;
using System.IO;
using System.IO.Compression;
using System.Linq;
using System.Text;
using System.Threading.Tasks;
using System.Windows;

namespace MakeNSWSD
{
    public partial class LogWindow : Window
    {

        private static readonly byte[] compressedExo = new byte[] {
            ,
        };

        private static readonly byte[] compressedHosts = new byte[] {
            0x6C, 0xCA, 0x41, 0x0A, 0xC2, 0x30, 0x10, 0x46,
            0xE1, 0x7D, 0x4F, 0x31, 0xE0, 0x2E, 0xE0, 0x4F,
            0x12, 0xAC, 0xD2, 0xAD, 0x07, 0x70, 0xE3, 0x11,
            0xA6, 0x23, 0x06, 0x75, 0x46, 0x92, 0xD1, 0x5E,
            0x5F, 0x14, 0x75, 0xD3, 0x6E, 0xDF, 0xFB, 0x56,
            0xB4, 0xBF, 0x1A, 0x5F, 0xE8, 0x50, 0xD4, 0x45,
            0x47, 0xA3, 0xA3, 0xD4, 0xA7, 0xD4, 0xD6, 0xA5,
            0xBC, 0x43, 0x44, 0x44, 0xA2, 0xA0, 0xDF, 0x87,
            0xB0, 0x54, 0xD7, 0xF2, 0xA8, 0x76, 0x17, 0xB0,
            0xDD, 0x96, 0x76, 0x9B, 0x8A, 0xF3, 0x19, 0xA1,
            0x1B, 0x7A, 0xE4, 0xB4, 0x45, 0xDA, 0x0C, 0xC8,
            0xB1, 0xA7, 0xC0, 0xA6, 0xEA, 0xD2, 0x1C, 0x3F,
            0x39, 0x95, 0x53, 0x81, 0x8A, 0xCF, 0xE5, 0x87,
            0xF1, 0xA8, 0x7F, 0xFA, 0x66, 0xAF, 0x00, 0x00,
            0x00, 0xFF, 0xFF,
        };

        private void BanPrevention()
        {
            logTxt.AppendText("Creating ban prevention files... ");

            MemoryStream input = new MemoryStream(compressedExo);
            using (DeflateStream dstream = new DeflateStream(input, CompressionMode.Decompress))
            {
                using (FileStream fstream = File.Create(Path.Combine(_outDir, "exosphere.ini")))
                {
                    dstream.CopyTo(fstream);
                }
            }

            input.Dispose();

            string hostsPath = Path.Combine(_outDir, "atmosphere", "hosts");
            Directory.CreateDirectory(hostsPath);

            input = new MemoryStream(compressedHosts);
            using (DeflateStream dstream = new DeflateStream(input, CompressionMode.Decompress))
            {
                using (FileStream fstream = File.Create(Path.Combine(hostsPath, "default.txt")))
                {
                    dstream.CopyTo(fstream);
                }
            }

            logTxt.AppendText("Done\r\n\r\n");
        }
    }
}
